...
async def handle_location(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message or not update.message.location:
        ...
        await update.message.reply_text(
            "⚠️ I couldn't get your location. Please turn on location sharing "
...
async def handle_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not lat or not lon:
        ...
        await query.message.reply_text(
            "⚠️ Please share your location first using /start.",
            reply_markup=reply_markup,
        )
        return
    ...
    await query.message.reply_text(
        f"🔍 Filtering by: *{label}*...", parse_mode="Markdown"
    )
...
async def search_and_send(update_or_query, context, lat, lon, filter_type="all"):
    ...
    if not places:
        await reply_target.reply_text(
            "😔 No places found nearby. Try a different filter or a wider area."
        )
        return
